...
def process_jobs(jobs_list : List[Job],start_date: date,end_date :date = None,location_country: CountryEnum = None,remote : bool = True) -> List[Job]:
    ...
    locations = []
    if location_country:
        locations = COUNTRY_TO_LOCATIONS[location_country.value]
        locations.append(location_country.value)
    
    for job in jobs_list:
        # if locations:
        #     print("matching")
        #     match = any(location not in job.location for location in locations)
        #     if not match:
        #         print(job.location)
